...
### Requirement: One shared right-hand panel

Self Review SHALL present one resizable, collapsible panel docked to the right of the diff. The panel SHALL have Changed files and GitHub comments tabs, with only the selected tab's content visible and keyboard-reachable. This arrangement SHALL apply at all supported widths, rather than introducing an overlay or bottom panel on narrow hosts.

#### Scenario: Initial review layout
- **WHEN** the user opens Self Review for a task
- **THEN** the shared right-hand panel is open with Changed files selected
...
- **THEN** the comment list replaces the changed-file content within the same panel
