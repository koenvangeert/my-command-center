...
async fn spawn_retry_contract(host: &impl PtyHost, cwd: &Path) {
...
async fn ordered_io_contract(host: &impl PtyHost, cwd: &Path) {
...
async fn existing_adapter_satisfies_spawn_retry_contract() {
    ...
    spawn_retry_contract(&host, fixture.directory.path()).await;
}
...
impl Drop for ExistingFixture {
    fn drop(&mut self) {
        // Cleanup also runs when an assertion panics; never use process-name cleanup.
