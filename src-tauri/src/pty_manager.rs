...
impl PtyManager {
    ...
    /// Spawns a new PTY process for the given task_id running `claude --resume <claude_session_id>`.
    /// This is interactive TUI mode — no output-format flags.
    ///
    /// # Arguments
    /// * `task_id` - Unique identifier for the task
    /// * `worktree_path` - Working directory for the claude process
    /// * `claude_session_id` - Claude session ID to resume
    /// * `cols` - Terminal width in columns
    /// * `rows` - Terminal height in rows
    /// * `app_handle` - Tauri app handle for emitting events
    ///
    ...
    pub async fn spawn_claude_pty(
        ...
        if sessions.contains_key(task_id) {
            ...
            // Clean up old PID file (will be recreated below)
            if let Ok(pid_dir) = self.get_pid_dir() {
                let _ = std::fs::remove_file(pid_dir.join(format!("{}-claude-pty.pid", task_id)));
            }
        }
        ...
        cmd.env("TERM", "xterm-256color");
        cmd.env("COLORTERM", "truecolor");
        // Inform Claude TUI that this is a VSCode-compatible terminal.
        // xterm.js does not support Kitty keyboard protocol or Kitty graphics,
        // so setting TERM_PROGRAM to "vscode" prevents unsupported escape sequences.
        cmd.env("TERM_PROGRAM", "vscode");
        ...
        let pid_dir = self.get_pid_dir()?;
        std::fs::create_dir_all(&pid_dir)?;
        let pid_file = pid_dir.join(format!("{}-claude-pty.pid", task_id));
        std::fs::write(&pid_file, pid.to_string())?;
        ...
        // Async emitter task: receives from channel, batches output, emits Tauri events.
        // Batch PTY output to reduce Tauri event frequency and prevent visual tearing.
        // Claude TUI redraws at 60 FPS; without batching, partial frames appear between
        // cursor-positioning and content writes. We flush at ~60 FPS (every 16ms) or
        // when the buffer exceeds 64KB.
        const FLUSH_INTERVAL_MS: u64 = 16;
        const MAX_BUFFER_SIZE: usize = 65536; // 64KB early flush threshold
