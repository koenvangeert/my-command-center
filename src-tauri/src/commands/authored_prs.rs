...
pub async fn fetch_authored_prs(
    ...
    for pr in &prs {
        ...
        enriched.insert(pr.id, (created_at, ci_status, ci_check_runs, review_status));
    }

    {
        ...
        if can_delete_stale && prs.len() >= search_item_count {
            db_lock
