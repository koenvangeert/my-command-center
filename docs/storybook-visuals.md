...
The runner waits for the pinned Storybook preview's interaction phase to finish and the declared readiness selector, then pauses runtime timers before waiting for fonts and paint. Timers remain live during interactions; afterward, controlled 16-millisecond steps let canvas writes settle without letting transient success messages expire during slow captures. Terminal input is blurred during settling to stop canvas-caret blinking; other controls retain focus. Consecutive identical frames are required. Interaction errors and unsettled output fail rather than producing a baseline.
